  ...
  install(Vue, options) {
    ...
    let CocheerWebSocket = {
      init: function (openid) {
        ...
        Vue.prototype.axios.get(`${Vue.prototype.HOST}/api/v1/relation/device_basic_info?openid=${openid}`)
          .then(res => {
            ...
            return Vue.prototype.axios.get(`${Vue.prototype.HOST}/api/v1/base/check_device_online?mac=${res.data.data.mac}`);
          }).then(resp => {
            if (resp) {
      ...
      onMessage: function (event) {
        ...
        /* ============================================================ */
        $store.dispatch('CHANGE_DEVICE_ID', data.device_id);
        $store.dispatch('CHANGE_STATE_CHILDLOCK', data.childlock);
        $store.dispatch('CHANGE_VOLUME', data.volume);
        $store.dispatch('CHANGE_MODE', data.lamp_mode);
        $store.dispatch("CHANGE_LIGHT_STATUS", data.led);
        $store.dispatch('CHANGE_LIGHTNESS', data.brightness);
        $store.dispatch('CHANGE_LED_ON_TIME', data.led_on_time);
        $store.dispatch('CHANGE_SIT_NORMAL_TIME', data.sit_normal_time);
        $store.dispatch('CHANGE_SIT_ERROR_TIME', data.sit_error_time);
        $store.dispatch('CHANGE_SIT_REMIND_COUNT', data.sit_remind_count);
        $store.dispatch('CHANGE_REST_REMIND_COUNT', data.rest_remind_count);
        /* ============================================================ */
          ...
          if ((Vue.prototype.constant.StateType[state] & data.state) != 0) {
            switch (state) {
              ...
              //   break;
                ...
                /* =============================================== */
                // case "LED_ON_TIME":
                //   console.log('data.led_on_time', data.led_on_time);
                //   $store.dispatch(`CHANGE_${state}`, data.led_on_time);
                //   break;
                // case "SIT_NORMAL_TIME":
                //   console.log(data.sit_normal_time)
                //   $store.dispatch(`CHANGE_${state}`, data.sit_normal_time);
                //   break;
                // case "SIT_ERROR_TIME":
                //   console.log('data.sit_error_time', data.sit_error_time);
                //   $store.dispatch(`CHANGE_${state}`, data.sit_error_time);
                //   break;
                // case "SIT_REMIND_COUNT":
                //   console.log(data.sit_remind_count)
                //   $store.dispatch(`CHANGE_${state}`, data.sit_remind_count);
                //   break;
                // case "REST_REMIND_COUNT":
                //   console.log('data.rest_remind_count', data.rest_remind_count);
                //   $store.dispatch(`CHANGE_${state}`, data.rest_remind_count);
                //   break;
