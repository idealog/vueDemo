<template>
	<div class="wrap">
		<h3 class="title">
			...
			<span class='finish_tip' @click.stop="setDate($event)">{{finish_tip}}</span>
		</h3>
		<!--选择属性-->
...
export default {
  ...
  methods: {
    ...
    setDate(e) {
      let [year, month, day] = [
        parseInt(this.showYear) + 1900,
        parseInt(this.showMonth) + 1,
        parseInt(this.showDay) + 1
      ];
      month = month < 10 ? "0" + month : month;
      day = day < 10 ? "0" + day : day;
      this.$emit("setDate", `${year}-${month}-${day}`);
      //this.hidePop(e)
    }
  }
};
